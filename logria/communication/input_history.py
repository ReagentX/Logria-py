...
class HistoryTape():
    ...
    def scroll_forward_n(self, num_to_scroll: int) -> str:
        ...
        """
        if self.history_tape:
            self.current_index = min(len(self.history_tape) - 1, self.current_index + num_to_scroll)
        return self.get_current_item()
    ...
    def __repr__(self):
        return f'<History Tape containing {len(self.history_tape):,} items, currently selected {self.get_current_item()}>'
